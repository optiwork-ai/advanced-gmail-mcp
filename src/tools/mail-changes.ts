...
export function registerGetMailChanges(server: McpServer): void {
  server.tool(
    ...
    + 'Returns { account, fromHistoryId, resumedFrom, historyId, complete, nextPageToken, added, '
    + 'deleted, labelsAdded, labelsRemoved, note }: added carries message summaries, the other '
    + 'three carry ids/threadIds plus the label ids the change involved. '
    + 'resumedFrom appears only when no history_id was passed, and carries the remembered '
    + 'history id this poll resumed from (the same value as fromHistoryId) — so it is visible '
    + 'which cursor the server had stored. '
    + 'The server remembers the returned historyId ONLY when complete is true, because the '
    + 'cursor it returns is the mailbox\'s current position rather than the end of this page — '
    + 'remembering it mid-pagination would skip the pages you have not read. So while complete '
