...
- **This works on a personal `@gmail.com` account too, not only on work accounts.** Every account set up on this machine was tried against Google before release, the consumer one included, and all five came back with a real room and a real link. If some account ever is refused a room, the answer says the room could not be attached and the event is still there.
...
- Repeat alerts can be kept together — but do it by pointing at the conversation, not by the key. Every post tells you which conversation it landed in; pass that back on the next one and the run stays in one place. There is also a "key of your own" option, and it is left in because Google offers it, but do not lean on it: Google describes that key as belonging to the app that set it, and this server posts as you rather than as an app, so whether a second message with the same key joins the first conversation is unconfirmed — and if it does not, Chat quietly starts a new conversation instead of complaining. The answer now says exactly that rather than promising the messages will collect.
- **A message cannot notify a whole room by accident.** Chat's message body is not inert text: a particular scrap of markup in it (`<users/all>`) notifies every member of the space, and alert text is routinely assembled from something that was just read — an email, a file, a log line. Quoted text like that is now defused before posting, so it reads as ordinary words and nobody is paged; the answer says how many were defused, and there is a switch to send it as written when the mention is the point.
- **A failed post says whether it may have gone out anyway.** The post is never retried automatically, because a retry is a second message in front of everybody. But a gateway failure can arrive after Chat has already accepted the message, and "service unavailable" on its own invites a retry that posts the duplicate. The failure now says the message may already be in the space, names the space to check, and gives a key to retry under that makes Chat return the message it already created instead of posting another.
- Refusals happen before anything is posted, never halfway through: an empty message, a message longer than Chat's 4,096-character limit (with the actual length named so you know how much to cut), a conversation that belongs to a different space, asking for a conversation two contradictory ways at once, and a space, conversation or message id that is not one — a web link or a display name pasted in where an id belongs is now met with a sentence saying so, instead of a mangled request and a Google error nobody can act on. Emoji count as one character each, the way a person counts them.
...
- **Drive search now finds files in shared (team) drives — and you can open them.** They were invisible, with nothing saying so — an empty result reads exactly like "that file does not exist". My Drive and every shared drive the account can see are now searched together; `include_shared_drives: false` narrows it back. Reading a file works the same way, so a search hit from a team drive opens instead of coming back as "file not found". Results carry `driveId` so a shared-drive file can be told apart. No new permission is needed.

- **`create_google_doc` (1 tool): Claude can now MAKE a document, not just edit one.** "Write that up as a doc" produces a real document in your Drive and hands back its link — title, optional starting text, optional folder. It needs **no new permission and no re-consent**: the document is created through Drive, using the `drive.file` grant added on 2026-08-27, so it works on any account already authenticated since then. That grant is also the whole safety story — it reaches only files this server itself created, never the rest of your Drive. The starting text is plain text, so Markdown arrives as literal characters rather than formatting; `update_google_doc` takes it from there.
- **`update_google_doc` (1 tool), the first write this server makes to a Google Doc.** It does two things and refuses to do more: add text at the end of a document, and replace text you can name, everywhere it appears. Both in one call if you like — the replacements run first, so text you add is not rewritten by them.
...
- **The `documents` permission replaces `documents.readonly`, and each account must re-consent before it can WRITE.** It covers reading as well, so both Docs tools travel on the one grant from now on. Reading does not break in the meantime: a token keeps the permissions it was issued with, and Google accepts the older `documents.readonly` for reads, so **`get_google_doc` goes on working untouched**. Only **`update_google_doc` answers 403 until that alias runs `npm run auth -- <alias>` again**, and it says exactly that rather than reporting a broken login.
...
- **The mail watcher remembers where it got to.** Asking "what has arrived since last time?" no longer means keeping a bookmark yourself: `get_mail_changes` called with no cursor continues from where that account was last read to. A session that forgets, or a scheduled job that restarts, picks up exactly where the last one stopped instead of starting blind. The first poll on an account still needs one cursor from `get_history_baseline`; after that it is remembered, per account, on this machine.
  ...
  - **A cursor from a different account is never written down.** Handing this one a bookmark belonging to another mailbox is the mistake it is built to survive: it says so, and it leaves the bookmark it was keeping exactly where it was. A far-future value stored once could never have been corrected, and the account would have reported "no new mail" for ever after.
...
### Fixed
- **An emailed picture came back as an unnamed blob instead of a picture.** Gmail hands out a different internal id for the same attachment every time a message is read, so by the time the file was fetched the id no longer matched anything in the message — and the fetch, which still returned the right bytes, could no longer say what they were. The result was a file called "attachment" of unknown type: the viewable-image feature above never fired on real mail, and a file saved to disk lost its real name. The attachment is now identified by its position in the message, which does not change, with the id and a size match as backups. `read_email` reports that position as `partId` and `get_attachment` takes it as `part_id`; when nothing can identify the part, the result now says so in a note instead of presenting a placeholder name as fact.
- **A permission error from Chat, Drive or Docs told you to log in again when that was not the problem.** (Every one of those tools, reading a Drive file included — where the commonest refusal of all is "you do not have access to this file", which logging in again cannot fix.) The real causes are a permission never granted, an API never switched on for the Cloud project, or a file the account simply cannot see — none of them fixed by re-authenticating, which sent you round a loop while the true cause survived only in a tail of the message. Each now names what is actually wrong and what fixes it. A 401, where logging in again really is the answer, is untouched.
